 ...
 * externalise the node builtins, so `join` and even `createRequire` came back
 * undefined and the whole suite failed to load — on Windows only, which made
 * it look like a platform quirk for weeks. The converters only need
 * `DOMParser`, so providing just that is both smaller and portable.
 */
...
import {
  ...
  convertEpub,
  promoteTableHeaders
} from '../src/lib/convert/office'
import { htmlToMarkdown } from '../src/lib/convert/html'
