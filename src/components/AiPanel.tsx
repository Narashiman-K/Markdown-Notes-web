...
export default function AiPanel(props: AiPanelProps): React.JSX.Element {
  ...
  const [keyState, setKeyState] = useState<Record<string, boolean> | null>(null)
  ...
  const refreshStatus = useCallback(async (p: ProviderId) => {
    ...
    const s = await platform.aiStatus(p)
    setStatus(s)
    if (s.ready) {
      const m = await platform.aiModels(p)
  ...
  useEffect(() => {
    ...
    platform.keyState().then(setKeyState)
  }, [refreshStatus])
  ...
  useEffect(() => {
    ...
    if (!container) return
    const questions = container.querySelectorAll('.ai-turn.user')
    const anchor = questions[questions.length - 1] as HTMLElement | undefined
    ...
    // Measure against the container's own box rather than offsetTop, which is
    // relative to the nearest positioned ancestor and gives the wrong origin
    // whenever that is not this element.
    const top = anchor.getBoundingClientRect().top - container.getBoundingClientRect().top
    container.scrollTo({ top: container.scrollTop + top - 8, behavior: 'smooth' })
  }, [turns, busy])

  const changeProvider = async (p: ProviderId): Promise<void> => {
    setProvider(p)
    await platform.setSettings({ aiProvider: p })
    await refreshStatus(p)
  }

  const changeModel = async (m: string): Promise<void> => {
    setModel(m)
    await platform.setSettings({ aiModel: m })
  }
  ...
  const saveKey = async (): Promise<void> => {
    ...
    setKeyDraft('')
    setKeyState(await platform.keyState())
  ...
  const send = async (mode: AiMode, question: string): Promise<void> => {
    ...
    const res = await platform.aiChat({
      provider,
      model,
  ...
  const needsKey = provider !== 'ollama' && !keyState?.[provider]
        ...
        <div className="ai-settings">
          ...
          {provider === 'ollama' ? (
            ...
            <div className="key-block">
              ...
              <button className="link" onClick={() => platform.openExternal(KEY_HELP[provider].url)}>
